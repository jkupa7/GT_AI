...
def max_player(game, time_left, depth_left):
    ...
    valid_moves = get_valid_moves(True, game)
    ...
    game_over, winner = is_terminal(game)
    if game_over:
        if winner == None:
            return None, 1/2
        else:
...
def min_player(game, time_left, depth_left):
    ...
    valid_moves = get_valid_moves(False, game)
...
def is_terminal(game):
    ...
    am = game.get_active_moves()
    im = game.get_inactive_moves()

    if len(am) == 0 and len(im) == 0:
        return True, None
    if len(am) == 0:
        return True, game.get_inactive_player()
    return False, None
...
def heuristic(game, player):
    ...
    print(f"Player moves: {player_moves} -- Opponent moves: {opponent_moves}")
    return len(player_moves) - len(opponent_moves)




def get_valid_moves(active, game):

    if active:
        valid_moves = game.get_active_moves()
    else:
        valid_moves = game.get_inactive_moves()
    return valid_moves
...
######################################################################
##### CODE BELOW IS USED FOR RUNNING LOCAL TEST DON'T MODIFY IT ######͏︍͏︆͏󠄁
# tests.name_of_the_test #you can uncomment this line to run your test͏︍͏︆͏󠄁
################ END OF LOCAL TEST CODE SECTION ######################͏︍͏︆͏󠄁
